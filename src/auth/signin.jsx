...
const Logins = (props) => {
  ...
  const loginWithJwt = (username, password) => {
    axios.post(getToken, { username, password }).then(response => {
      ...
      axios.get(getUserDetail, { headers: { Authorization: "Bearer " + response.data.token } }).then(user => {
        var userRole = "user";
        if (user.data.company_admin !== undefined) { userRole = "companyadmin" }
        if (user.data.user_superuser) { userRole = "superuser" }
        setValue(man);
        setName(user.data.user_firstname + " " + user.data.user_lastname);
        localStorage.setItem('authenticatedUser', JSON.stringify({ ...user.data, token: response.data.token, user_role: userRole }));
